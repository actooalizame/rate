...
var subsEditProf = new SubsManager();
...
Router.route('/edit-review/:_id', {
	...
	name: 'editReview',
	data: function(){
		var review = Profreviews.findOne(this.params._id),
				professorId = review.professorId,
				professor = Professors.findOne({_id:professorId});
		return review;
	},
	waitOn: function(){
		var reviewId = this.params._id,
				review = Profreviews.findOne(this.params._id);
				professorId = review.professorId;
				console.log(reviewId);
				console.log(professorId);
		return [
			subsEditProf.subscribe('singleProfessor', professorId),
      subsEditProf.subscribe('professorReviews', professorId),
      subsEditProf.subscribe('userData')
     ];
	}
});
...
Router.route('/professor/:_id', {
	...
	waitOn: function(){
		...
		var profName = Session.get('profName');
		if(profSchool!==null){
			return [
				subsc.subscribe('multipleProfessors', profSchool,profName),
...
var subsAllProfReviews = new SubsManager();
...
Router.route('allProfessors', {
	...
	waitOn: function(){
		...
		return [
			subsAllProfReviews.subscribe('shortProfessors'),
			subsAllProfReviews.subscribe('Profreviews')
		];
...
Router.route('myReviews', {
	...
	waitOn: function(){
		if(Meteor.user()){
			return [
				subsd.subscribe('myProfReviews'),
				subsd.subscribe('mySchoolReviews'),
				subsd.subscribe('shortProfessors'),
				subsd.subscribe('shortSchools')
			];
		}
		else {
			Router.go('home');
...
Router.route('suggestProfessor', {
	template: 'suggestProfessor',
	loadingTemplate: "loading",
	path: '/sugerir-profesor'
...
var clearMultipleProfessorSession = function() {
	Session.set('profSchool', undefined);
	Session.set('profName', undefined);
};

Router.onBeforeAction(requireLogin, {only: ['rate','editReview','rateProfessor','myReviews']});

Router.onStop(clearMultipleProfessorSession, {only: ['viewProfessor']});
